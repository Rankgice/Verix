...
- 运行 gRPC 调试依赖 `grpcurl` 在 `PATH` 中可用。
...
- `headers`：HTTP 下断言响应 header；gRPC 下断言 response initial metadata。
...
## Workspace 全局变量
调用 `initialize_testspec` 后会在项目根目录生成 `.mcp/global.json`：
...
- `vars` 为用户自定义的自由结构。
- 变量优先级：`.mcp/global.json vars < spec.vars < extract`。
- `{{vars.xxx}}`、`{{timestamp}}` 语法保持不变。
